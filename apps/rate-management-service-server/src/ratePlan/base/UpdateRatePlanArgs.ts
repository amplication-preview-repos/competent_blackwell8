/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { RatePlanWhereUniqueInput } from "./RatePlanWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { RatePlanUpdateInput } from "./RatePlanUpdateInput";

@ArgsType()
class UpdateRatePlanArgs {
  @ApiProperty({
    required: true,
    type: () => RatePlanWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => RatePlanWhereUniqueInput)
  @Field(() => RatePlanWhereUniqueInput, { nullable: false })
  where!: RatePlanWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => RatePlanUpdateInput,
  })
  @ValidateNested()
  @Type(() => RatePlanUpdateInput)
  @Field(() => RatePlanUpdateInput, { nullable: false })
  data!: RatePlanUpdateInput;
}

export { UpdateRatePlanArgs as UpdateRatePlanArgs };
